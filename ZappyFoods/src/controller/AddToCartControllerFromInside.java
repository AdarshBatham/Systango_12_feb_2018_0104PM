...
/**
 * Servlet implementation class AddToCartControllerFromInside
 */
@WebServlet("/AddToCartControllerFromInside")
public class AddToCartControllerFromInside extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddToCartControllerFromInside() {
        super();
		...
		String dateonly=date1+"/"+month+"/"+year;
						...
						if(quantity!=0)
						{
							...
							{*/
											...
											if(x==1)
											{
												RequestDispatcher rd=request.getRequestDispatcher("view_all_products_from_inside.jsp");/*forwarded to "view_all_products.jsp"  with the data shown below:===="calculated_values","m2","all_products_arraylist"*/
												ArrayList<Cart> calculated_values=new ProductModel().calculateValues(customeripaddress);
												System.out.println("AddToCartControllerFromInside---->doPost()----->calculated_values="+calculated_values);
						...
						{
							RequestDispatcher rd=request.getRequestDispatcher("view_all_products_from_inside.jsp");
